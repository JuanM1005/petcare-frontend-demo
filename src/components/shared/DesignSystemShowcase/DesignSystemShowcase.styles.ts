export const showcasePage = 'mx-auto max-w-[960px] px-6 py-8';

export const showcaseHeader =
  'mb-10 flex items-center gap-6 rounded-2xl bg-linear-to-br from-blue-600 to-blue-400 p-8 shadow-lg';

export const showcaseLogo =
  'h-16 w-16 rounded-xl bg-white p-[0.15rem] object-contain shadow-lg';

export const showcaseHeaderText = 'flex flex-col gap-2';

export const showcaseTitle =
  'text-2xl font-semibold tracking-[-0.02em] text-white';

export const showcaseVersionBadge =
  'ml-2 rounded-full bg-white/90 px-2 py-[2px] align-middle text-xs font-medium text-blue-600';

export const showcaseDescription = 'text-sm text-white/80';

export const showcaseSection =
  'mb-10 rounded-xl border border-slate-200 bg-white p-6 shadow-sm';

export const showcaseSectionTitle =
  'mb-6 rounded-lg border-l-4 border-blue-500 bg-slate-100 px-4 py-3 text-xl font-semibold text-slate-900';

export const showcaseSectionContent = 'flex flex-col gap-6';

export const showcaseSubsection = 'flex flex-col gap-3';

export const showcaseSubsectionTitle =
  'text-sm font-medium tracking-[0.05em] text-slate-500 uppercase';

export const showcaseRow = 'flex flex-wrap items-center gap-3';
